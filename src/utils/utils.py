...
async def get_chatgpt_response(message, type):
    ...
    body = {
        ...
        "messages": [
            ...
            {"role": "system", "content": "respond in 1-2 sentences exactly, you are a DJ, " + ("setting" if type == "first_message" else "changing") + " the music to be associated with the user's mood, example: sad music for sad mood (DO NOT RESPOND TO THE USER WITH ANY ARTIST, ALBUM, OR SONG)"},
            {"role": "user", "content": message}
        ]
    }
    try:
        ...
        error_message = e
        log.error(error_message)
        return {"status": 401, "error": error_message}